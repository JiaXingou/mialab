...
#####################序列和representation
    def __init__(self,path,path_repre,path_label,path_contact,flag,transform=None):
    ...
    def __getitem__(self,index):             ####index的这个特征
        ...
        ###########得到attention数据
        if self.flag == 1:
            with open(os.path.join('./example', name + '_attention.pkl'), 'rb') as fo:  # 读取pkl文件数据
            ...
            with open(os.path.join('./valid_3', name + '.pkl'), 'rb') as fo:  # 读取pkl文件数据
...
class model_model(nn.Module):
    ...
    def forward(self, repre,attention_map,hot,G):
        ...
        feature=torch.cat((out_2_repre,attention_map,hot),axis=3)  #attention_map是1，208，208，660     
...
def main():
    ...
    train = Dataset(path_train,train_h5fi_repre,train_h5fi_label,train_h5fi_contact,flag=1)
    #test = Dataset(path_test, test_h5fi_repre,test_h5fi_label,test_h5fi_contact,flag=0)
    #test_28 = Dataset(path_test_28, test_28_h5fi_repre, test_28_h5fi_label, test_28_h5fi_contact,flag=3)
    ...
    #testloader_28 = DataLoader(dataset=test_28, batch_size=batch_size, num_workers=num_workers, collate_fn=_collate_fn)
    ...
    for epoch in range(0, epoch):
        ...
        val_sum_loss = 0.0
        test_sum_loss = 0.0
        for batch_idx,(repre,name,seq,label,attention,hot,G,mask) in enumerate(trainloader):          #repre是1，m,1280,label是1，m,m
            da=name+seq
            da=[da]
            length = len(trainloader)
            optimizer.zero_grad()
            repre, attention,hot,G,mask=repre[0].to(device), attention[0].to(device),hot[0].to(device),G[0].to(device),mask[0].to(device)
            repre, attention, hot,mask=repre.unsqueeze(0), attention.unsqueeze(0),hot.unsqueeze(0),mask.unsqueeze(0)
            # G=G[0]
            outputs=my_model(repre,attention,hot,G)            #repre是1,208,1280，attention是1，208，208，660
...
#############测试
        ...
        with torch.no_grad():
            ...
            for batch_idx, (repre, name, seq, label, attention, hot, G,mask) in enumerate(
                ...
                da = name + seq
                da = [da]
                repre, attention, hot, G,mask = repre[0].to(device), attention[0].to(device), hot[0].to(device), G[0].to(
                    device),mask[0].to(device)
                repre, attention, hot ,mask= repre.unsqueeze(0), attention.unsqueeze(0), hot.unsqueeze(0), mask.unsqueeze(0)
                outputs = my_model(repre, attention, hot, G)  # repre是1,208,1280，attention是1，208，208，660
                outputs = outputs.squeeze(0)
                ######################改，7_9
        ...
        ######################################################
            ...
            for batch_idx, (repre, name, seq, label, attention, hot, G,mask) in enumerate(
                    testloader_28):  # repre是1，m,1280,label是1，m,m
                da = name + seq
                da = [da]
                repre, attention, hot, G,mask = repre[0].to(device), attention[0].to(device), hot[0].to(device), G[0].to(
                    device),mask[0].to(device)
                ...
                outputs = outputs.cpu().detach().numpy()
                label = label.cpu().detach().numpy()
                relax_0_28.append(calculateEvaluationStats(outputs, label, label.shape[0], name))
                del label
                torch.cuda.empty_cache()
            ...
            T10 = float(T10)
            L30 = float(L30)
            print('[epoch:%d] test_loss: %.06f| T10: %.03f' % (epoch + 1, test_loss_epoch, T10))
            print('[epoch:%d] test_loss: %.06f| L10: %.03f' % (epoch + 1, test_loss_epoch, L30))
